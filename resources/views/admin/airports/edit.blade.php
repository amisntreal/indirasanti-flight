        ...
        <form action="{{ route('admin.airports.update', $airport) }}" method="POST">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div class="col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $airport->name) }}" required class="w-full px-4 py-2.5 border border-amber-200 rounded-lg text-sm focus:outline-none focus:border-amber-400">
                </div>
                <div>
                    <label class="block text-xs font-medium text-amber-700 mb-1.5 uppercase tracking-wide">Kota</label>
                    <input type="text" name="city" value="{{ old('city', $airport->city) }}" required class="w-full px-4 py-2.5 border border-amber-200 rounded-lg text-sm focus:outline-none focus:border-amber-400">
                </div>
                <div>
                    <label class="block text-xs font-medium text-amber-700 mb-1.5 uppercase tracking-wide">Negara</label>
                    <input type="text" name="country" value="{{ old('country', $airport->country) }}" required class="w-full px-4 py-2.5 border border-amber-200 rounded-lg text-sm focus:outline-none focus:border-amber-400">
                </div>
                <div>
                    <label class="block text-xs font-medium text-amber-700 mb-1.5 uppercase tracking-wide">Kode IATA</label>
                    <input type="text" name="iata_code" value="{{ old('iata_code', $airport->iata_code) }}" required maxlength="5" class="w-full px-4 py-2.5 border border-amber-200 rounded-lg text-sm focus:outline-none focus:border-amber-400 uppercase">
                </div>
            </div>
